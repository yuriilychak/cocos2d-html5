...
export class TextFieldTTF extends LabelTTF {
  ...
  constructor(placeholder, dimensions, alignment, fontName, fontSize) {
    super();
    this.colorSpaceHolder = new Color(127, 127, 127);
    this._colorText = new Color(255, 255, 255, 255);
  ...
  deleteBackward() {
    ...
    if (
      ...
      this.delegate.onTextFieldDeleteBackward(
        this,
        this._inputText[strLen - deleteLen],
        deleteLen
      )
    ) {
  ...
  insertText(text, len) {
    ...
    if (sInsert.length > 0) {
      if (
        this.delegate &&
        this.delegate.onTextFieldInsertText(this, sInsert, sInsert.length)
      ) {

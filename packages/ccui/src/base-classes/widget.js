...
export class Widget extends ProtectedNode {
  constructor() {
    ...
    this._touchBeganPosition = new Point();
    this._touchMovePosition = new Point();
    this._touchEndPosition = new Point();
    this._widgetType = Widget.TYPE_WIDGET;
    this._customSize = new Size();
    this._layoutParameterDictionary = {};
    this._sizeType = Widget.SIZE_ABSOLUTE;
    this._sizePercent = new Point();
    this._positionType = Widget.POSITION_ABSOLUTE;
    this._positionPercent = new Point();
    this._nodes = [];
  ...
  get widthPercent() {
    return this._getWidthPercent();
  }
  set widthPercent(v) {
    this._setWidthPercent(v);
  }

  get heightPercent() {
    return this._getHeightPercent();
  }
  set heightPercent(v) {
  ...
  _isAncestorsEnabled() {
    ...
    if (parentWidget && !parentWidget.enabled) return false;
  ...
  _getAncensterWidget(node) {
    ...
    if (parent instanceof Widget) return parent;
    else return this._getAncensterWidget(parent.parent);
  }
  ...
  updateSizeAndPosition(parentSize) {
    ...
    switch (this._sizeType) {
      case Widget.SIZE_ABSOLUTE:
        ...
        this._sizePercent.y =
          parentSize.height > 0
            ? this._customSize.height / parentSize.height
            : 0;
        break;
      case Widget.SIZE_PERCENT:
        var cSize = new Size(
          parentSize.width * this._sizePercent.x,
    ...
    switch (this._positionType) {
      ...
      case Widget.POSITION_PERCENT:
        absPos = new Point(
          parentSize.width * this._positionPercent.x,
          parentSize.height * this._positionPercent.y
        );
        break;
      default:
        break;
  ...
  setSizeType(type) {
    ...
    if (this._usingLayoutComponent) {
      var component = this._getOrCreateLayoutComponent();
      component.setUsingPercentContentSize(
        this._sizeType === Widget.SIZE_PERCENT
      );
    }
  }
  ...
  getWorldPosition() {
    return this.transformComponent.convertToWorldSpace(
      new Point(
  ...
  findNextFocusedWidget(direction, current) {
    if (
      ...
      null == this.onNextFocusedWidget(direction)
    ) {
  ...
  onFocusChange(widgetLostFocus, widgetGetFocus) {
    if (widgetLostFocus) widgetLostFocus.focused = false;
    if (widgetGetFocus) widgetGetFocus.focused = true;
  }
  ...
  dispatchFocusEvent(widgetLostFocus, widgetGetFocus) {
    ...
    if (widgetGetFocus !== widgetLostFocus) {
      ...
      ServiceLocator.eventManager.dispatchEvent(
        new EventFocus(widgetLostFocus, widgetGetFocus)
      );
    }
  }
  ...
  hitTest(pt) {
    ...
    return Rect.containsPoint(bb, this.transformComponent.convertToNodeSpace(pt));
  }
  ...
  _getNormalGLProgram() {
    return ServiceLocator.shaderCache.get(
  ...
  if (enable) {
    if (null == Widget._focusNavigationController) {
      ...
      if (Widget._focusedWidget) {
        Widget._focusNavigationController._setFirstFocsuedWidget(

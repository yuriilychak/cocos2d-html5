...
export class Scale9Sprite extends EventHelper(Node) {
  ...
  _updateCapInsets(rect, capInsets) {
    ...
    if (!Rect.equalToZero(rect)) {
      ...
      this._insetRight =
        rect.width - this._insetLeft - this._capInsetsInternal.width;
      this._insetBottom =
        rect.height - this._insetTop - this._capInsetsInternal.height;
    }
  }
  ...
  initWithSpriteFrameName(spriteFrameName, capInsets) {
    if (!spriteFrameName)
      throw new Error(
        "Scale9Sprite.initWithSpriteFrameName(): spriteFrameName should be non-null"
      );
    capInsets = capInsets || new Rect();

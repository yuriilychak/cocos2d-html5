...
export class LayoutComponent extends Component {
  ...
  setAnchorPosition(point, y) {
    var oldRect = this.owner.getBoundingBox();
    this.owner.setAnchorPoint(point, y);
    var newRect = this.owner.getBoundingBox();
  ...
  refreshLayout() {
    ...
    switch (this._horizontalEdge) {
      ...
      case LayoutComponent.horizontalEdge.RIGHT:
        ...
        ownerPosition.x =
          parentSize.width -
          (this._rightMargin + (1 - ownerAnchor.x) * ownerSize.width);
        break;
      case LayoutComponent.horizontalEdge.CENTER:
        if (this._isPercentOnly) break;
        if (this._usingStretchWidth) {
          ownerSize.width =
            parentSize.width - this._leftMargin - this._rightMargin;
    ...
    switch (this._verticalEdge) {
      case LayoutComponent.verticalEdge.NONE:
        if (this._usingStretchHeight && !this._isPercentOnly) {
          ...
          ownerPosition.y =
            this._bottomMargin + ownerAnchor.y * ownerSize.height;
        } else {
          if (this._usingPositionPercentY)
      ...
      case LayoutComponent.verticalEdge.TOP:
        ...
        ownerPosition.y =
          parentSize.height -
          (this._topMargin + (1 - ownerAnchor.y) * ownerSize.height);
        break;
      case LayoutComponent.verticalEdge.CENTER:
        if (this._isPercentOnly) break;
        if (this._usingStretchHeight) {
          ownerSize.height =
            parentSize.height - this._topMargin - this._bottomMargin;
  ...
  _getOwnerParent() {
    return this.owner ? this.owner.parent : null;
  }
  ...
  _refreshHorizontalMargin() {
    ...
    this._rightMargin =
      parentSize.width - (ownerPoint.x + (1 - ownerAnchor.x) * ownerSize.width);
  }
  ...
  _refreshVerticalMargin() {
    ...
    this._topMargin =
      parentSize.height -
      (ownerPoint.y + (1 - ownerAnchor.y) * ownerSize.height);
  }
}

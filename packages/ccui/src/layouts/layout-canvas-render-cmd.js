...
export class LayoutCanvasRenderCmd extends ProtectedNodeCanvasRenderCmd {
  ...
  stencilClippingVisit(parentCmd) {
    var node = this._node;
    if (!node._clippingStencil || !node._clippingStencil.visible) return;
...
LayoutCanvasRenderCmd.prototype.scissorClippingVisit =
  LayoutCanvasRenderCmd.prototype.stencilClippingVisit;
LayoutCanvasRenderCmd.prototype.postScissorVisit =
  LayoutCanvasRenderCmd.prototype.postStencilVisit;

LayoutCanvasRenderCmd._getSharedCache = function () {
  return (
    ClippingNode._sharedCache ||
    (ClippingNode._sharedCache = document.createElement("canvas"))
  );
};

...
export class Layout extends Widget {
  ...
  set clippingEnabled(v) {
    this.setClippingEnabled(v);
  }

  set clippingType(v) {
    this.setClippingType(v);
  }
  ...
  set layoutType(v) {
    this.setLayoutType(v);
  }

  onEnter() {
    super.onEnter();
    if (this._clippingStencil)
      this._clippingStencil.performRecursive(NodeStateCallbackType.onEnter);
    this._doLayoutDirty = true;
    this._clippingRectDirty = true;
  }
  ...
  onExit() {
    super.onExit();
    if (this._clippingStencil)
      this._clippingStencil.performRecursive(NodeStateCallbackType.onExit);
  }
          ...
          case Widget.RIGHT:
            if (this._isLastWidgetInContainer(this, direction)) {
              if (this._isWidgetAncestorSupportLoopFocus(current, direction))
                return super.findNextFocusedWidget(direction, this);
              return current;
            } else return super.findNextFocusedWidget(direction, this);
          case Widget.DOWN:
            return this._getNextFocusedWidget(direction, current);
          case Widget.UP:
            return this._getPreviousFocusedWidget(direction, current);
    ...
    switch (this._clippingType) {
      ...
      case Layout.CLIPPING_STENCIL:
        if (able) {
          ...
          if (this.running)
            this._clippingStencil.performRecursive(
  ...
  _getClippingRect() {
    if (this._clippingRectDirty) {
      var worldPos = this.transformComponent.convertToWorldSpace(new Point(0, 0));
      var t = this.transformComponent.nodeToWorldTransform;
      ...
      if (this._clippingParent) {
        ...
        var right = Math.min(
          worldPos.x + scissorWidth,
          parentClippingRect.x + parentClippingRect.width
        );
        var top = Math.min(
          worldPos.y + scissorHeight,
          parentClippingRect.y + parentClippingRect.height
        );
  ...
  _supplyTheLayoutParameterLackToChild(locChild) {
    ...
    switch (this._layoutType) {
      ...
      case Layout.LINEAR_VERTICAL:
        var layoutParameter = locChild.getLayoutParameter(
          LayoutParameter.LINEAR
        );
        if (!layoutParameter)
  ...
  setBackGroundColorType(type) {
    ...
    switch (this._colorType) {
      ...
      case Layout.BG_COLOR_SOLID:
        ...
        break;
      case Layout.BG_COLOR_GRADIENT:
        this._gradientRender = new LayerGradient(
  ...
  _getLayoutAccumulatedSize() {
    ...
    if (type === Layout.LINEAR_HORIZONTAL)
      layoutSize.height =
        layoutSize.height -
        (layoutSize.height / widgetCount) * (widgetCount - 1);

    if (type === Layout.LINEAR_VERTICAL)
      layoutSize.width =
        layoutSize.width - (layoutSize.width / widgetCount) * (widgetCount - 1);
    return layoutSize;
  }
  ...
  _findNearestChildWidgetIndex(direction, baseWidget) {
    ...
    var distance = FLT_MAX,
      found = 0;
    if (
      direction === Widget.LEFT ||
      direction === Widget.RIGHT ||
      direction === Widget.DOWN ||
      direction === Widget.UP
    ) {
      ...
      while (index < count) {
        ...
        if (w && w instanceof Widget && w.isFocusEnabled()) {
          var length =
            w instanceof Layout
              ? w._calculateNearestDistance(baseWidget)
              : Point.vectorLength(
                  Point.sub(this._getWorldCenterPoint(w), widgetPosition)
                );
          if (length < distance) {
            found = index;
            distance = length;
    ...
    if (
      ...
      while (index < count) {
        ...
        if (w && w instanceof Widget && w.isFocusEnabled()) {
          var length =
            w instanceof Layout
              ? w._calculateFarthestDistance(baseWidget)
              : Point.vectorLength(
                  Point.sub(this._getWorldCenterPoint(w), widgetPosition)
                );
          if (length > distance) {
    ...
    for (var i = 0, len = locChildren.length; i < len; i++) {
      ...
      if (widget instanceof Layout)
        ...
        if (widget instanceof Widget && widget.isFocusEnabled())
          length = Point.vectorLength(
            Point.sub(this._getWorldCenterPoint(widget), widgetPosition)
          );
        else continue;
      }
  ...
  _findProperSearchingFunctor(direction, baseWidget) {
    ...
    var widgetPosition = this._getWorldCenterPoint(
      this._findFirstNonLayoutWidget()
    );
    if (direction === Widget.LEFT) {
      this.onPassFocusToChild =
        previousWidgetPosition.x > widgetPosition.x
      ...
      this.onPassFocusToChild =
        previousWidgetPosition.y > widgetPosition.y
          ? this._findNearestChildWidgetIndex
          : this._findFarthestChildWidgetIndex;
  ...
  _getWorldCenterPoint(widget) {
    var widgetSize =
      widget instanceof Layout
        ...
        : widget.contentSize;
    return widget.transformComponent.convertToWorldSpace(
  ...
  _copySpecialProperties(layout) {
    ...
    this.setBackGroundImage(
      layout._backGroundImageFileName,
      layout._bgImageTexType
    );

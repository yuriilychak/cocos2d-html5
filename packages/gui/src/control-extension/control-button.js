...
export class ControlButton extends Control {
  ...
  set adjustBackground(v) {
    this.setAdjustBackgroundImage(v);
  }
  get preferredSize() {
  ...
  get labelAnchor() {
    return this.getLabelAnchorPoint();
  }
  set labelAnchor(v) {
    this.setLabelAnchorPoint(v);
  }
  ...
  constructor(label, backgroundSprite, fontSize) {
    ...
    if (fontSize != null)
      this.initWithTitleAndFontNameAndFontSize(
        label,
        backgroundSprite,
        fontSize
      );
    ...
    else if (label != null) this.initWithBackgroundSprite(label);
  }
  ...
  needsLayout() {
    ...
    if (label) label.color.color = this._currentTitleColor;
  ...
  initWithLabelAndBackgroundSprite(label, backgroundSprite) {
    if (!label)
      throw new Error(
        "ControlButton.initWithLabelAndBackgroundSprite(): label should be non-null"
      );
    if (!backgroundSprite)
      throw new Error(
        "ControlButton.initWithLabelAndBackgroundSprite(): backgroundSprite should be non-null"
      );
    if (super.init(true)) {
      ...
      this.setTitleColorForState(label.color.color, CONTROL_STATE_NORMAL);

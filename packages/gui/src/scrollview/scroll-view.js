...
export class GScrollView extends Layer {
  ...
  get maxOffset() {
    return this.maxContainerOffset();
  }
  get bounceable() {
    return this.isBounceable();
  }
  set bounceable(v) {
    this.setBounceable(v);
  }
  get viewSize() {
  ...
  get clippingToBounds() {
    return this.isClippingToBounds();
  }
  set clippingToBounds(v) {
    this.setClippingToBounds(v);
  }
  ...
  onTouchBegan(touch, event) {
    ...
    if (
      locTouches.length > 2 ||
      this._touchMoved ||
      !Rect.containsPoint(frame, locPoint)
    )
      return false;
  ...
  onTouchMoved(touch, event) {
    ...
    if (this._touches.length === 1 && this._dragging) {
      ...
      if (
        !this._touchMoved &&
        Math.abs(convertDistanceFromPointToInch(dis)) < MOVE_INCH
      ) {
        return;
      }
  ...
  setClippingToBounds(clippingToBounds) {
  ...
  _relocateContainer(animated) {
    ...
    var newY = oldPoint.y;
    if (
      locDirection === SCROLLVIEW_DIRECTION_BOTH ||
      locDirection === SCROLLVIEW_DIRECTION_HORIZONTAL
    ) {
    ...
    if (
      locDirection === SCROLLVIEW_DIRECTION_BOTH ||
      locDirection === SCROLLVIEW_DIRECTION_VERTICAL
    ) {
  ...
  _deaccelerateScrolling(dt) {
    ...
    if (
      (Math.abs(locScrollDistance.x) <= SCROLL_DEACCEL_DIST &&

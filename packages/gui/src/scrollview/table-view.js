...
export class TableView extends GScrollView {
  ...
  get verticalFillOrder() {
    return this.getVerticalFillOrder();
  }
  set verticalFillOrder(v) {
    this.setVerticalFillOrder(v);
  }
  ...
  _indexFromOffset(offset) {
    ...
    if (index !== -1) {
      ...
      if (index > maxIdx) index = INVALID_INDEX;
    }
    return index;
  }
  ...
  updateCellAtIndex(idx) {
    if (
      idx === INVALID_INDEX ||
      idx > this._dataSource.numberOfCellsInTableView(this) - 1
    )
      return;
  ...
  reloadData() {
    ...
    for (var i = 0, len = locCellsUsed.size; i < len; i++) {
      ...
      cell.reset();
      if (cell.parent === locContainer) locContainer.removeChild(cell, true);
    }
  ...
  onTouchEnded(touch, event) {
    ...
    if (this._touchedCell) {
      ...
      tmpOrigin = this.parent.convertToWorldSpace(tmpOrigin);
      bb.x = tmpOrigin.x;
      bb.y = tmpOrigin.y;
  ...
  onTouchBegan(touch, event) {
    ...
    if (this._touches.length === 1) {
      ...
      else this._touchedCell = this.cellAtIndex(index);

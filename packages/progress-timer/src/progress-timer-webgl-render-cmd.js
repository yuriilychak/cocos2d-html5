...
export class ProgressTimerWebGLRenderCmd extends Node.WebGLRenderCmd {
  ...
  updateStatus() {
    ...
    if (locFlag & flags.textureDirty) {
      this._updateProgressData();
      this._dirtyFlag =
        (this._dirtyFlag & flags.textureDirty) ^ this._dirtyFlag;
    }
  }
  ...
  initCmd() {
    if (!this._vertexData) {
      ...
      this._vertexArrayBuffer = new ArrayBuffer(
        MAX_VERTEX_COUNT * vertexDataLen
      );
      this._float32View = new Float32Array(this._vertexArrayBuffer);
      this._vertexData = [];
      for (let i = 0; i < MAX_VERTEX_COUNT; i++) {
        this._vertexData[i] = new V3F_C4B_T2F(
          null,
          null,
          null,
  ...
  _updateProgressData() {
    if (!this._vertexData) this.initCmd();
    if (!this._vertexData) return;
    const node = this._node;
    const locType = node._type;
    if (locType === TYPE_RADIAL) this._updateRadial();
    else if (locType === TYPE_BAR) this._updateBar();
    this._vertexDataDirty = true;
  }
  ...
  _updateBar() {
    ...
    const min = Point.sub(node._midPoint, alphaOffset),
      max = Point.add(node._midPoint, alphaOffset);
  ...
  _updateRadial() {
    ...
    if (alpha === 0) {
      ...
      for (i = 0; i <= locProTextCoordsCount; ++i) {
        const pIndex =
          (i + (locProTextCoordsCount - 1)) % locProTextCoordsCount;
      ...
      hit = Point.add(
        locMidPoint,
        Point.mult(Point.sub(percentagePt, locMidPoint), min_t)
      );
    }
    ...
    if (this._transformUpdating || !sameIndexCount) {
      ...
      for (i = 0; i < index; i++) {
        ...
        this._textureCoordFromAlphaPoint(
          locVertexData[i + 2].texCoords,
          alphaPoint.x,
          alphaPoint.y
        );
        this._vertexFromAlphaPoint(
          locVertexData[i + 2].vertices,
          alphaPoint.x,
          alphaPoint.y
        );
      }
    ...
    this._textureCoordFromAlphaPoint(
      locVertexData[this._vertexDataCount - 1].texCoords,
      hit.x,
      hit.y
    );
    this._vertexFromAlphaPoint(
      locVertexData[this._vertexDataCount - 1].vertices,
      hit.x,
      hit.y
    );
  }
  ...
  _boundaryTexCoord(index) {
    if (index < TEXTURE_COORDS_COUNT) {
      ...
      if (this._node._reverseDirection)
        return new Point(
          (locProTextCoords >> (7 - (index << 1))) & 1,
          (locProTextCoords >> (7 - ((index << 1) + 1))) & 1
        );
      else
        return new Point(
          (locProTextCoords >> ((index << 1) + 1)) & 1,
          (locProTextCoords >> (index << 1)) & 1
        );
    }
    return new Point();
  }
